...
int set_background_darwin(const char* path, int style) {
  @autoreleasepool {
    ...
    NSURL* image = [NSURL fileURLWithPath:
    ...
    for (NSScreen* screen in screens) {
      NSMutableDictionary* opts = [[sw desktopImageOptionsForScreen:screen]
        mutableCopy];
      if (style == STYLE_CENTER) {
        [opts setObject:
          [NSNumber numberWithInt:NSImageScaleNone]
        ...
        [opts setObject:
          [NSNumber numberWithInt:NSImageScaleAxesIndependently]
          forKey:NSWorkspaceDesktopImageScalingKey];
        [opts setObject:
      ...
      bool success = [sw setDesktopImageURL:
        image forScreen:screen
        options:opts
        error:&err];
      if (!success) {
        return E_FAIL;
